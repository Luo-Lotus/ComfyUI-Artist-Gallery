...
- 按分类管理 Prompt 和 Prompt 组合，支持搜索、封面与批量操作
...
在节点界面中选择 Prompt、分类或组合，并按分区组织输出。
...
图库中可以管理分类、Prompt 和组合，也可以进行以下操作：
...
打开 `设置 > 图库设置`，执行一次 `自动匹配封面`。它只处理当前没有封面的 Prompt 和组合。
